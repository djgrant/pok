...
- A node can optionally declare a `mount` function that _produces_ children.
...
The important part is not _how_ it loads, but that it plugs into the command tree the same way.
...
- be a general-purpose task runner (pok already has tasks; plugins are about _command discovery and composition_)
