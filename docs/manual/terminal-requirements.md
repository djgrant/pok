...
pok renders Unicode symbols, ANSI colors, prompt UIs, and fullscreen adapters. The terminal needs to support those features for the default reporter and TUI adapters to render correctly.
...
Setting `CI=true` switches pok to `--no-tty` behavior by default.
