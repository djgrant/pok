...
/**
 * Vite plugin to bundle @pokit/core and dependencies for WebContainer.
 *
 * At build time, this plugin:
 * 1. Uses bun build to bundle @pokit/core, reporter-clack, prompter-clack
...
function pokBundlePlugin(): Plugin {
  ...
  async function buildBundle() {
    ...
    console.log('[pok-bundle] Building @pokit/core bundle for WebContainer...');

    // Bundle @pokit/core with all dependencies
...
`;
    ...
    execSync(
      `bun build "${coreEntryPath}" --outfile "${path.join(outDir, 'core.js')}" --target node --format cjs --external zod --external fast-glob --external @pokit/reporter-clack --external @pokit/prompter-clack`,
      { cwd: rootDir, stdio: 'inherit' }
    );

    // Bundle reporter-clack
    execSync(
      `bun build "${reporterDir}/src/index.ts" --outfile "${path.join(outDir, 'reporter-clack.js')}" --target node --format cjs --external @pokit/core --external zod`,
      { cwd: rootDir, stdio: 'inherit' }
    );

    // Bundle prompter-clack
    execSync(
      `bun build "${prompterDir}/src/index.ts" --outfile "${path.join(outDir, 'prompter-clack.js')}" --target node --format cjs --external @pokit/core --external zod`,
      { cwd: rootDir, stdio: 'inherit' }
    );
    ...
    bundledFiles = {
      'node_modules/@pokit/core/dist/index.js': convertDynamicImportsToRequire(
        fs.readFileSync(path.join(outDir, 'core.js'), 'utf-8')
      ),
      'node_modules/@pokit/reporter-clack/dist/index.js': convertDynamicImportsToRequire(
        fs.readFileSync(path.join(outDir, 'reporter-clack.js'), 'utf-8')
      ),
      'node_modules/@pokit/prompter-clack/dist/index.js': convertDynamicImportsToRequire(
    ...
    bundledFiles['node_modules/@pokit/core/package.json'] = JSON.stringify(
      {
        name: '@pokit/core',
        version: '0.0.1',
        main: './dist/index.js',
    ...
    bundledFiles['node_modules/@pokit/reporter-clack/package.json'] = JSON.stringify(
      {
    ...
    bundledFiles['node_modules/@pokit/prompter-clack/package.json'] = JSON.stringify(
      {
...
`;
    ...
    bundledFiles['node_modules/@pokit/core/bin/pok.js'] = `#!/usr/bin/env node
const { runCli } = require('@pokit/core');
